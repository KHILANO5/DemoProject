...
async function handlePost(req: AuthRequest, res: NextApiResponse) {
    try {
        ...
        const { title, description, category, size, item_condition, images, points_value = 50 } = req.body;

        // Validation
        if (!title || !description || !category || !size || !item_condition) {
            return res.status(400).json({ error: 'All fields are required' });
        }

        // Insert item
        const [result] = await pool.query(
            `INSERT INTO items (user_id, title, description, category, size, item_condition, images, points_value, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'available')`,
            [userId, title, description, category, size, item_condition, JSON.stringify(images || []), points_value]
        );
